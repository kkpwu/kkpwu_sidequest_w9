...
let isMoonGravity = false; // track if moon gravity is active
let showDebug = false; // toggle debug mode with H key
...
function draw() {
  // --- TOGGLE MOON GRAVITY (G KEY) ---
  if (kb.presses("g")) {
    isMoonGravity = !isMoonGravity;
    world.gravity.y = isMoonGravity ? MOON_GRAVITY : GRAVITY;
  }

  // --- TOGGLE DEBUG MODE (H) ---
  if (kb.presses("h")) {
    showDebug = !showDebug;
    allSprites.forEach((s) => (s.debug = showDebug));
  }
  ...
  if (showDebug) {
    ...
    textSize(9);
    textAlign(LEFT, TOP);
    text(`[G] Gravity: ${isMoonGravity ? "MOON" : "EARTH"}`, 10, 10);
    text(`[H] Debug Mode: ON`, 10, 22);
    text(`Player X: ${nf(player.x, 1, 1)} Y: ${nf(player.y, 1, 1)}`, 10, 34);
    text(`FPS: ${floor(frameRate())}`, 10, 46);
  }
  camera.on();
