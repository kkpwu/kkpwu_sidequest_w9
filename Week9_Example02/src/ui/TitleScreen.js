...
export class TitleScreen {
  ...
  _wireAttackListener() {
    ...
    this._attackUnsub = this._game.events.on("player:attackWindow", (info) =>
      this._tryHitLetter(info),
    );
  }
  ...
  _tryHitLetter(info) {
    ...
    const rangeX = Number(this.pkg.tuning?.player?.attack?.rangeX ?? 20);
    const rangeY = Number(this.pkg.tuning?.player?.attack?.rangeY ?? 16);
    const player = this._game?.level?.player;
    if (!player?.sprite) return;

    const playerFeetY = y + (player.sprite.h ?? 12) / 2;
    ...
    for (const s of this._letterSprites) {
      ...
      if (Math.abs(dx) > rangeX + (s.w ?? 16) / 2) continue;

      const letterFeetY = s.y + (s.h ?? 16) / 2;
      if (Math.abs(letterFeetY - playerFeetY) > rangeY + 10) continue;

      // Hit! Destroy this letter with knockback effect
      this._destroyLetter(s, facing);
  ...
  _destroyLetter(s, facingDir) {
    ...
    s.rotation = facingDir * 15;
    s.life = 30; // p5play auto-removes after this many frames
    s._destroyed = true;
  }
